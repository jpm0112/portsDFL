...
def test_time_split_is_disjoint_and_exhaustive(toy_df):
    ...
    # Disjoint + exhaustive: row counts must sum to the original (none dropped/duplicated).
    assert len(train) + len(test) == len(toy_df)
    # Split by time, with no future leaking into training.
...
def test_encode_categoricals_dense_zero_indexed(toy_df):
    ...
    # Indices must be exactly 0,1,...,k-1 (dense, no gaps).
...
def test_apply_encoding_train_has_no_oov(toy_df):
    ...
    # The encoding was fit on train, so every train index must be non-OOV.
...
def test_add_log_target_exact():
    ...
    # log(e) == 1 exactly, so it's a clean check value.
...
def test_prepare_end_to_end(tmp_path, toy_df):
    ...
    # Toy dataset needs the columns covariate computation expects. .copy() avoids
    # mutating the shared fixture object for other tests.
    df = toy_df.copy()
    ...
    # Encoding level counts must match the number of unique train categories.

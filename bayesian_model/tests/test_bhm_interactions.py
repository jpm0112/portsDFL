...
def _generate_synthetic() -> tuple[pd.DataFrame, Encoding]:
    ...
    # Full N_VESSEL x N_BERTH grid of interaction offsets is the structure M4
    # is meant to recover.
...
def test_m4_recovers_interactions():
    ...
    # Low threshold so most cells in the 4x4 grid get an interaction term: with
    # 1500 rows over 16 cells (~94 each), every cell clears min_interaction_n=20,
    # so all planted effects are estimable.
    model = build_model(
    ...
    # If even the 5th percentile sits above 0.05, the posterior is confidently
    # away from zero, meaning the model detected real interactions.

...
TRUE_SIGMA = 0.35
# Two coefficients are exactly 0.0 to check the model can conclude "no effect".
...
def _generate_synthetic() -> tuple[pd.DataFrame, Encoding, CovariateScaler]:
    ...
    # Z's column order must stay aligned with `feat`, which matches TRUE_BETA's order.
    ...
    # Identity scaler (means 0, stds 1): standardization is a no-op, so the z_*
    # columns ARE the raw covariates and beta_sd stays directly interpretable.
...
def test_m1_recovers_beta_and_taus():
    ...
    # atol=0.07 also passes for the two true-zero betas.

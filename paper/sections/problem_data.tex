...
therefore a candidate source of target leakage. We address it directly in
Section~\ref{sec:results} by re-fitting every model with the feature removed:
doing so raises tree-model error by only about three percent and does not change
the ranking, so we report those leakage-free figures throughout and the predictive
conclusions do not depend on the suspect feature. Its exact measurement time should
nonetheless be confirmed with the port, since the small gap could be legitimate
pre-berthing signal rather than leakage. Second, the exact

...
def _pick(df, aliases, role: str, required: bool = True):
    ...
    # Walk the candidate names in priority order; return the first match.
    for name in aliases:
...
def _resolve_service_mask(selector, df_week, id_col) -> np.ndarray:
    ...
    """
    n = len(df_week)
    # Case 1: no selector -> nobody is a service.
    if selector is None:
        return np.zeros(n, dtype=bool)
    # Case 2: a callable. reshape(n) raises loudly on a wrong-length result.
    if callable(selector):
        return np.asarray(selector(df_week), dtype=bool).reshape(n)
    sel = list(selector)
    # Case 3: already a length-N boolean mask (every element a bool).
    is_bool_like = len(sel) == n and all(
        isinstance(v, (bool, np.bool_)) for v in sel
    )
    if is_bool_like:
        return np.asarray(sel, dtype=bool)
    # Case 4: a collection of vessel IDs. Needs an id column to match against.
    if id_col is None:
        raise ValueError(
...
def _build_bundle(
    *,
    ...
    # Raise if any vessel type matches no berth (allow_unmatched=False).
    compat = vessel_berth_compat(vessel_types, berths, allow_unmatched=False)

    if big_m is None:
        # Auto-size big-M: upper bound on any start time = latest arrival plus
...
def build_weekly_instance(
    ...
    *,
    ...
    # .copy() a passed-in DataFrame so later operations never mutate the caller's.
    ...
    # Keep an explicitly-passed column name, else auto-detect via _pick.
    ...
    # ws=week start, we=week end (exclusive).
    arr = pd.to_datetime(df[arrival_col])
    ws = pd.to_datetime(week_start)
    we = ws + pd.Timedelta(days=week_days)
    # Half-open [ws, we) so back-to-back weeks never double-count a midnight arrival.
    mask = (arr >= ws) & (arr < we)
    ...
    # Use the id column if found, else fall back to integer positions.
    vessel_ids = (
    ...
    # tau MUST be exactly (N,) so it index-aligns with the vessels.
    ...
    # Default: base_weight everywhere, then overwrite service rows with
    # service_weight. Or take a caller array/callable instead.
    if weights is None:
    ...
    # Latest-start window lᵢ: +inf (no window) for all, then lᵢ = arrival + slack
    # for service vessels.
...
def generate_synthetic_weekly_instance(
    ...
    *,
    ...
    # Sample vessel types from the port's rough mix; renormalise so the
    # probability vector sums to exactly 1 (required by rng.choice).
    ...
    # finish service before the horizon), then sorted chronologically.

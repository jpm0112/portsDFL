...
def assemble_schedule(
    ...
    for i in range(bundle.n_vessels):
        ...
        # Window respected if started no later than latest allowed start.
        # Non-service vessels have latest = +inf, so always True. +_EPS absorbs noise.
...
def compute_kpis(
    ...
    finish = starts + tau
    waits = np.maximum(0.0, starts - arrivals)  # floored at 0

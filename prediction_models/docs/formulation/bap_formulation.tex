...
% Color palette (shared with meetings/latex/dfl_explainer.tex)
...
The MILP lives in \texttt{src/ports\_dfl/optim/discrete\_bap.py}
...
\texttt{berth\_compat}, \texttt{service}) is in \texttt{optim/instance.py}; the
berth catalog and compatibility in \texttt{optim/berths.py}; the pre-solve
weekly slicing in \texttt{optim/weekly\_instance.py}; and the deterministic
planner in \texttt{scripts/plan\_week.py}. Open modeling decisions and
assumptions are tracked in \texttt{docs/formulation/decisions\_and\_questions.md}.

...
# Lazy TabM import inside the fixture so a missing torch/TabM dependency only
# breaks tests that use it. scope="module": built once per file.
...
def test_forward_pass_shape(tabm_cls, tiny_arrays) -> None:
    ...
    # One value per row: shape (n_rows, 1).
...
def test_backward_pass_produces_gradients(tabm_cls, tiny_arrays) -> None:
    ...
    # At least one gradient means backprop is wired up and the network trainable.
    assert n_grads > 0
...
def test_save_load_roundtrip(tabm_cls, first_fold_arrays, tmp_path) -> None:
    ...
    # Reloaded model must reproduce the original predictions. Looser rtol than
    # RealMLP because torch tensors serialize with slightly more float jitter.

...
from typing import Any  # `Any` = "value of any type" (used in the return-type hint below)

import optuna


# Each function takes an Optuna `trial` and returns a dict of chosen hyperparameters.
# The `trial.suggest_*` calls are how Optuna picks a value within a range/choice and
# records it; the sampler uses past results to choose smarter values over time.
def suggest_linear(trial: optuna.Trial) -> dict[str, Any]:
    """Hyperparameter search space for the linear (Ridge) model."""
    return {
        # suggest_float(name, low, high, log=True): pick a float in [low, high].
        # `log=True` samples on a log scale, good for learning rates that span
        # orders of magnitude (1e-4 ... 1e-1). `1e-4` means 0.0001.
        "lr": trial.suggest_float("lr", 1e-4, 1e-1, log=True),
        "weight_decay": trial.suggest_float("weight_decay", 1e-5, 1e0, log=True),
        # suggest_categorical(name, choices): pick exactly one value from the list.
...
def suggest_realmlp(trial: optuna.Trial) -> dict[str, Any]:
    ...
    return {
        ...
        # suggest_int(name, low, high): pick an integer in [low, high] (both inclusive).
        "depth": trial.suggest_int("depth", 2, 4),
        # Dropout offered as fixed choices (incl. 0.0 = no dropout) rather than a range.
...
def suggest_node(trial: optuna.Trial) -> dict[str, Any]:
    ...
    """
    return {
        # `step=2` restricts choices to 2, 4, 6 (every 2nd integer from low to high).
        ...
        # The choice function controls how the trees route samples; tuned here while
        # the bin function stays fixed (see the docstring above).

...
def _evaluate_best(best_params: dict, X, y, splits, max_epochs: int, out_dir: Path) -> pd.DataFrame:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splits):
        ...
        # Fresh preprocessor per fold, fit on train only, to prevent val leaking
        # into the transform.
        ...
        model = TabM(input_dim=X_train.shape[1], max_epochs=max_epochs,
...
def main() -> None:
    ...
    # Closure over args.max_epochs so every tuned model uses the CLI epoch count.
    ...
    # Results persist to a SQLite DB so runs are resumable; SEED makes it reproducible.
    study = run_study(
    ...
    # Pass a copy so _evaluate_best can pop/mutate without corrupting stored params.

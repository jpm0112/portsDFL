...
import optuna
import pandas as pd
# Alias to avoid confusion with this repo's own Ridge.
...
def _evaluate_best(
    ...
    # Pull out the preprocessing-only choices so what remains is model hyperparameters.
    cat_strategy = best_params.pop("categorical_strategy", "target")
    scaler = best_params.pop("numeric_scaler", "standard")

    fold_metrics = []  # PyTorch Ridge
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splits):
        ...
        # Fresh preprocessor per fold, fit on train only, to prevent leakage from val.
...
def main() -> None:
    ...
    # Pass a copy so _evaluate_best's .pop() calls don't mutate Optuna's best_params.

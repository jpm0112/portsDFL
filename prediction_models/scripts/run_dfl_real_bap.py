...
def _berth_utilization(
    ...
    """
    n_berths = x_assign.shape[1]
    busy_per_berth = np.zeros(n_berths)
    for i in range(len(starts)):
        for b in range(n_berths):
            if x_assign[i, b] > 0.5:
                busy_per_berth[b] += tau[i]
...
def _mean_wait(starts: np.ndarray, arrivals: np.ndarray) -> float:
    ...
    # Clamp negatives to 0 so early starts aren't counted as "negative wait".
            ...
            rows.append(
                {
                    ...
                    # how much worse the prediction-driven plan is vs the optimum
...
def main() -> None:
    ...
    # Fit the encoding on TRAIN only; val is transform-only to avoid data leakage.
    ...
    # --- Build instances ---------------------------------------------------
    # Different seeds for train vs val so the two instance sets differ.
    ...
    out_dir = RESULTS_DIR / "dfl_real_bap"
    ...
    pto_preds = predict_pto(pto_module, X_val)
    pto_pred_metrics = all_metrics(y_val, pto_preds)
    ...
    # Warm-start DFL from the trained PtO weights instead of from scratch.

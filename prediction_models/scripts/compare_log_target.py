...
# Factories (not pre-built objects) so each run gets a brand-new model instance.
MODEL_FACTORIES = {
...
def main() -> None:
    ...
    train_idx, val_idx = splits[0]  # use only fold 0 for this quick test
    ...
    for name, factory in MODEL_FACTORIES.items():
        ...
        # Same model wrapped to train/predict on log1p(target): the other half
        # of the ablation, log-transformed target vs raw target.
        log_metrics = evaluate_one(
            LogTargetWrapper(factory(n_features)), Xt, yt, Xv, yv
        )
        rows.append({"model": name, "target": "log", **log_metrics})

    table = pd.DataFrame(rows)  # long-format: one row per (model, target)
    # Reshape long -> wide: one row per model, columns split by target per metric.
    ...
    # How much log-target changed each metric vs raw.
    ...
    # No index=False here, so the "model" row labels ARE written to the CSV.
    pivot.to_csv(out)
    print(f"\nWritten to {out}")

"""Tune and CV-evaluate the RandomForest benchmark (Predict-then-Optimize, not DFL).

Mirrors run_xgb.py but stays on plain TPE: RandomForest is a single fit with no
boosting rounds, so multi-fidelity pruning buys nothing and its tuning surface is
flat (hence the small search space in suggest_rf).

Usage:
    python scripts/run_rf.py [--n_trials 30]
"""
...
# Quiet Optuna's mixed-categorical UserWarning (suggest_rf uses None/str/float
# choices), matching the other ML run scripts (run_realmlp / run_tabm / run_node).
...
from ports_dfl.tuning.search_spaces import suggest_rf
...
def main() -> None:
    ...
    parser.add_argument("--study_name", type=str, default="rf")
    ...
    # --- Stock defaults ----------------------------------------------------
    print("Evaluating RandomForest with stock defaults...")
    ...
    # --- Tuned: plain TPE (no pruning — RF has no rounds to prune over) ----
    print(f"Running Optuna study (TPE) with {args.n_trials} trials...")
    objective = make_objective(
        factory=RandomForestRegressorModel,
        suggest_fn=suggest_rf,

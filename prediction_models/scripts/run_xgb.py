"""Tune and CV-evaluate the XGBoost benchmark (Predict-then-Optimize, not DFL).

Mirrors run_realmlp.py: a stock-defaults run plus an Optuna study, then a
full-metrics re-evaluation of the best configuration. Tuning uses TPE (Bayesian)
with a Hyperband pruner; the objective reports running-mean MAE per fold so the
pruner can stop clearly-losing configs early (per-config round economy comes from
XGBoost's own early stopping inside each fit).

Usage:
    python scripts/run_xgb.py [--n_trials 30]
"""
...
# Quiet library chatter (xgboost) and Optuna's mixed-categorical UserWarning,
...
from ports_dfl.tuning.search_spaces import suggest_xgb
...
def main() -> None:
    ...
    parser.add_argument("--study_name", type=str, default="xgb")
    ...
    out_dir = RESULTS_DIR / "xgb"
    out_dir.mkdir(parents=True, exist_ok=True)

    # --- Stock defaults ----------------------------------------------------
    print("Evaluating XGBoost with stock defaults...")

...
# setdefault respects a value the user may have already set.
...
def _evaluate(name: str, model_kwargs: dict, X, y, splits, out_dir: Path) -> pd.DataFrame:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splits):
        ...
        # Fresh preprocessor each fold, fit on train only, so val never leaks
        # into the encoding/scaling.
        ...
        # Distinct seed per fold so folds are independent draws (merge overrides any
        # random_state already in model_kwargs without a duplicate-kwarg error).
        model = RealMLP(input_dim=X_train.shape[1], **{**model_kwargs, "random_state": SEED + fold_idx})
...
def main() -> None:
    ...
    # Reference run with built-in defaults (only n_epochs overridden), no tuning.
    ...
    # Closure over args.n_epochs forces every tuned model to use the CLI epoch count.
    ...
    # Pass a copy so the helper can't mutate Optuna's best_params. NOTE: unlike the
    # tuning study above (which forced n_epochs via `factory`), this re-evaluation
    # does NOT pass n_epochs, so the tuned re-run uses RealMLP's default epoch count.
    # See REPORTED findings.

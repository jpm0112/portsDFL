...
PAGE_SIZE = (8.5, 11)  # US Letter, portrait — width x height in inches per PDF page
...
# Returns the first file that loads successfully.
...
def _load_all_data() -> dict:
    ...
    # Maps a short name -> loaded table (or None if missing). Pages look these up.
    return {
...
def _model_summary_row(df: pd.DataFrame | None, model_name: str) -> dict | None:
    ...
    if df is None:
        return None
    # First column (the fold label, e.g. "mean"/"std") becomes the row index.
...
def _build_predictive_table(data: dict) -> pd.DataFrame:
    ...
    if data["baselines"] is not None:
        for _, br in data["baselines"].iterrows():
            # Tidy up the baseline name for display.
...
# Create one blank figure = one PDF page.
...
def _table_axes(fig, rect, df: pd.DataFrame, header_color: str = "#cfd8dc",
    ...
    table.auto_set_font_size(False)  # so our fontsize sticks
    table.set_fontsize(8)
    table.scale(1.0, 1.4)  # stretch rows taller for readability
    # Style the header row: grey background + bold text.
...
def _page_title(pdf, data: dict):
    # Fall back to an empty dict so .get() calls stay safe when config.json is missing.
...
def _page_predictive(pdf, data: dict):
    ...
    # Display copy so string-formatting numbers doesn't alter the real table.
    show = table.copy()
    ...
    # Blue for real models, grey for baselines.
    ...
    ax_mae.invert_yaxis()  # best row at the top
    ...
    ax_mape.set_yticklabels([])  # shared with the MAE chart on the left
...
def _page_dfl_method(pdf, data: dict):
    ...
    if trace is not None and len(trace) > 0:
        ...
        ax1.grid(alpha=0.3)
        # Second y-axis so loss and regret (different scales) share one chart.
        ax2 = ax1.twinx()
...
def _page_three_objectives(pdf, data: dict):
    ...
    # Sort by FI cost ascending; apply the same order to all three arrays so the
    # instances stay aligned across curves.
    order = np.argsort(fi)
    ...
    # Regret = decision cost minus the full-information optimum.
...
def _page_regret_distribution(pdf, data: dict):
    ...
    for arr, color, label in [(pto_r, "#d62728", "PtO"), (dfl_r, "#1f77b4", "DFL")]:
        ...
        # Running fraction of points seen: 1/n, 2/n, ..., n/n.
    ...
    stats = pd.DataFrame(
        {
            "PtO": [
                ...
                f"{pto_r.min():.2f}",
                f"{pto_r.max():.2f}",
                # percentage of instances with positive regret
...
def _page_decision_quality(pdf, data: dict):
    ...
    for k, label in [
        ...
        if k in dec.columns and len(dec) >= 2:
            ...
            # Pct improvement (negative = DFL better); guard against PtO value of 0.
            deltas.append(100 * (dfl_v - pto_v) / pto_v if pto_v != 0 else 0)
    # Blue if DFL improved (delta <= 0), red if it got worse.
...
def _runtime_table_and_chart(pdf):
    ...
    if cache.exists():
        ...
        # Hard-coded fallback (kept in sync with benchmark_dbb.py).
        bench = pd.DataFrame(
            {
                "N": [5, 6, 8, 8, 10],
                "M": [2, 2, 2, 3, 3],
        ...
        # Cache the fallback numbers so the next run reads them from disk.
    ...
    # Log y-axis since values span orders of magnitude.
    ...
    # Annotate each point with its berth count (M).
...
def _page_conclusions(pdf, data: dict):
    ...
    dec = data["decision"]
    # Default to None; filled in below only if the data exists.
    ...
    if pto_regret is not None:
        # diff < 0 means DFL has lower regret; pick wording by gap size.
